...
- `datetime_debut` — date et heure de mise en place sur la plaque, saisi au démarrage PWA. `DATETIME` pour permettre le suivi multi-jours. Reconstitué backend : date du jour + heure saisie.
...
| Transition en_repos→production | `PUT /demarrer` accepte heure_debut, installation, tare_kg. Stocke `datetime_debut = date_du_jour + heure`. Seuls les champs fournis sont mis à jour. |
...
- ✅ Bug api.php corrigé — $data passé à demarrer()
- ✅ LotController.php demarrer() — datetime_debut + tare_kg obligatoire
