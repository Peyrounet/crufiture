-- ============================================================
-- Module /crufiture — Schema SQL v3
-- Base : u191509486_dbboutique (partagée Peyrounet)
-- Préfixe : cruf_*
-- Généré le : 2026-03-30
-- Mis à jour le : 2026-04-04 (v3 — workflow lots v2)
-- ============================================================
...
CREATE TABLE cruf_lot (
    ...
    -- ── Production réelle ──────────────────────────────────────
    poids_reel_kg       DECIMAL(8,3)    DEFAULT NULL,     -- poids réel total mis en jarres
...
-- Permet le suivi des sorties par jarre (hors scope v1).
-- ------------------------------------------------------------
...
--
-- poids_brut_kg stocke le poids NET (tare déjà déduite côté
-- frontend — la tare n'est pas stockée en BDD).
-- reste_evap_kg = poids_brut_kg - cible_kg du lot.
...
CREATE TABLE cruf_releve_evaporation (
    ...
    lot_id          INT UNSIGNED    NOT NULL,
    heure           TIME            NOT NULL,
    poids_brut_kg   DECIMAL(8,3)    NOT NULL,        -- poids net plateau (tare déjà déduite)
    reste_evap_kg   DECIMAL(8,3)    DEFAULT NULL,    -- calculé : poids_brut_kg - cible_kg
...
--    ├── cruf_jarre (1-3)           stockage après production (max 3 jarres)
