 ...
 * @php 7.4+
 *
 * v4 (2026-04-05) — PWA mobile production :
 *   - demarrer()  : accepte heure_debut, installation, tare_kg depuis $data
 *   - addReleve() : météo structurée (temperature, humidite, vent_kmh, ensoleillement)
 *   - stocker()   : jarres avec tare_kg + poids_pleine_kg, poids_initial_kg calculé,
 *                   sans limite de nombre de jarres
 *   - getOne()    : inclut tare_kg sur le lot, nouveaux champs météo + jarres
 *   - getSuivi()  : inclut tare_kg
 *   - castLotRow(): cast tare_kg
 */
...
class LotController
{
    ...
    // Inclut tare_kg pour le calcul côté PWA.
    // ─────────────────────────────────────────────────────────────
    public function getSuivi()
    {
        $result = $this->mysqli->query(
            "SELECT l.id, l.numero_lot, l.date_production, l.statut,
                    l.tare_kg, l.cible_kg, l.evaporation_kg, l.masse_totale_kg,
                    l.fructose_kg, l.saccharose_kg,
                    s.nom AS saveur_nom,
                    (SELECT rv.poids_brut_kg FROM cruf_releve_evaporation rv
                     WHERE rv.lot_id = l.id ORDER BY rv.heure DESC, rv.id DESC LIMIT 1) AS dernier_poids_net,
                    (SELECT rv.heure FROM cruf_releve_evaporation rv
                     WHERE rv.lot_id = l.id ORDER BY rv.heure DESC, rv.id DESC LIMIT 1) AS dernier_releve_heure,
                    (SELECT COUNT(*) FROM cruf_releve_evaporation rv WHERE rv.lot_id = l.id) AS nb_releves
             FROM cruf_lot l
             JOIN cruf_saveur s ON s.id = l.saveur_id
        ...
        while ($row = $result->fetch_assoc()) {
            $row['id']               = (int)   $row['id'];
            $row['tare_kg']          = $row['tare_kg']          !== null ? (float) $row['tare_kg']          : null;
    ...
    // ─────────────────────────────────────────────────────────────
    // GET /lots/:id
    // Fiche complète : lot + fruits + relevés + contrôles + jarres.
    // ─────────────────────────────────────────────────────────────
    public function getOne($id)
    {
        ...
        // Relevés d'évaporation — météo structurée
        $stmt3 = $this->mysqli->prepare(
            "SELECT id, heure, poids_brut_kg, reste_evap_kg,
                    temperature, humidite, vent_kmh, ensoleillement,
                    remarque, created_at
        ...
        while ($row = $r3->fetch_assoc()) {
            ...
            $row['humidite']       = $row['humidite']       !== null ? (float) $row['humidite']       : null;
            $row['vent_kmh']       = $row['vent_kmh']       !== null ? (float) $row['vent_kmh']       : null;
            $row['ensoleillement'] = $row['ensoleillement'] !== null ? (int)   $row['ensoleillement'] : null;
            $releves[] = $row;
        }
        $stmt3->close();
        $lot['releves'] = $releves;
        ...
        // Jarres — avec tare_kg et poids_pleine_kg
        $stmt5 = $this->mysqli->prepare(
            "SELECT id, numero, tare_kg, poids_pleine_kg, poids_initial_kg, poids_actuel_kg, note, created_at
        ...
        while ($row = $r5->fetch_assoc()) {
            ...
            $row['tare_kg']          = $row['tare_kg']          !== null ? (float) $row['tare_kg']          : null;
            $row['poids_pleine_kg']  = $row['poids_pleine_kg']  !== null ? (float) $row['poids_pleine_kg']  : null;
    ...
    public function update($id, $data)
    {
        ...
        // Recalcul Krencker si paramètres présents
        ...
        if ($has_krencker) {
            ...
            if ($base > 0 && isset($brix_fruit) && $brix_fruit < $brix_cible) {
                ...
                foreach (['sucre_fruit_kg','sa_kg','fructose_kg','saccharose_kg','masse_totale_kg','evaporation_kg','cible_kg'] as $kf) {
                    $sets[]  = "$kf = ?";
                    $params .= 'd';
                    $vals[]  = $k[$kf];
                }
    ...
    public function mettreEnRepos($id)
    {
        ...
        if ((float) $lot['poids_base_kg'] <= 0) {
            echo ResponseHelper::jsonResponse('Les poids doivent être renseignés (blocs 2 et 3 incomplets).', 'error', null, 422);
            return;
        }
        ...
        $stmt2 = $this->mysqli->prepare("UPDATE cruf_lot SET statut = 'en_repos' WHERE id = ?");
        $stmt2->bind_param('i', $id);
        $stmt2->execute();
        $stmt2->close();
    ...
    // Accepte heure_debut, installation, tare_kg depuis la PWA.
    // Verrouille définitivement la fiche.
    // ─────────────────────────────────────────────────────────────
    public function demarrer($id, $data)
    {
        $id = (int) $id;
    ...
    public function stocker($id, $data)
    {
        ...
        // Vérifier >= 1 contrôle qualité existant ou fourni
        ...
        // Calculer poids_reel_kg = somme des contenus des jarres
        $poids_reel = 0.0;
        foreach ($data['jarres'] as $jarre) {
            $tare_j   = isset($jarre['tare_kg'])         && $jarre['tare_kg']         !== '' ? (float) $jarre['tare_kg']         : 0.0;
            $pleine_j = isset($jarre['poids_pleine_kg']) && $jarre['poids_pleine_kg'] !== '' ? (float) $jarre['poids_pleine_kg'] : 0.0;
            $contenu  = round($pleine_j - $tare_j, 3);
            if ($contenu > 0) $poids_reel += $contenu;
        }
        $poids_reel = round($poids_reel, 3);
        ...
        // Jarres — sans limite de nombre
        $num = 1;
        foreach ($data['jarres'] as $jarre) {
            $tare_j      = isset($jarre['tare_kg'])         && $jarre['tare_kg']         !== '' ? (float) $jarre['tare_kg']         : null;
            $pleine_j    = isset($jarre['poids_pleine_kg']) && $jarre['poids_pleine_kg'] !== '' ? (float) $jarre['poids_pleine_kg'] : null;
            $contenu     = ($tare_j !== null && $pleine_j !== null) ? round($pleine_j - $tare_j, 3) : 0.0;
            $note_j      = isset($jarre['note']) ? $jarre['note'] : null;

            $stmtJ = $this->mysqli->prepare(
                "INSERT INTO cruf_jarre (lot_id, numero, tare_kg, poids_pleine_kg, poids_initial_kg, poids_actuel_kg, note)
                 VALUES (?, ?, ?, ?, ?, ?, ?)"
            );
            $stmtJ->bind_param('iidddds', $id, $num, $tare_j, $pleine_j, $contenu, $contenu, $note_j);
            $stmtJ->execute();
            $stmtJ->close();
            $num++;
        }

        // Contrôle qualité fourni avec la mise en stock
    ...
    // Note obligatoire. Numéro de lot vérifié côté frontend (4 étapes).
    // ─────────────────────────────────────────────────────────────
    ...
    // poids_brut_kg reçu = poids NET (tare déduite côté frontend).
    // Météo structurée : temperature, humidite, vent_kmh, ensoleillement.
    // ─────────────────────────────────────────────────────────────
    public function addReleve($id, $data)
    {
        ...
        $heure         = isset($data['heure'])         && $data['heure']         !== '' ? $data['heure']         : date('H:i:s');
        $poids_net     = (float) $data['poids_brut_kg']; // poids net = tare déjà déduite côté frontend
        $cible         = (float) $lot['cible_kg'];
        ...
        $humidite      = isset($data['humidite'])      && $data['humidite']      !== '' ? (float) $data['humidite']      : null;
        $vent_kmh      = isset($data['vent_kmh'])      && $data['vent_kmh']      !== '' ? (float) $data['vent_kmh']      : null;
        $ensoleillement= isset($data['ensoleillement'])&& $data['ensoleillement']!== '' ? (int)   $data['ensoleillement']: null;

        $stmt2 = $this->mysqli->prepare(
            "INSERT INTO cruf_releve_evaporation
             (lot_id, heure, poids_brut_kg, reste_evap_kg, remarque,
              temperature, humidite, vent_kmh, ensoleillement)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
        );
        $stmt2->bind_param(
            'isddsdddi',
            $id, $heure, $poids_net, $reste_evap, $remarque,
            $temperature, $humidite, $vent_kmh, $ensoleillement
        );
        $stmt2->execute();
    ...
    private function saveFruits($lot_id, $fruits)
    {
        ...
        $ordre = 0;
        foreach ($fruits as $f) {
            if (empty($f['produit_id'])) continue;
